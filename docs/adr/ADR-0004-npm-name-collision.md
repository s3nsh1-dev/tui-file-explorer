...
package we have already decided never to publish (the maintainer chose _release-ready, never fired_
for Stage 3).
...
- **Rename everything to a free name now.** Real cost (the maintainer already chose `glim` twice —
  ...
  never published. The `§1` "rename in Stage 1" rule exists to stop _snapshot churn_, and deferring
