...
- **No alternate screen at all.** Leaves 40 screens of file listing in the user's scrollback after
  every run. `glim` is specified as a full-screen application.

## Verification

`glim | head` on a non-TTY stdout emits no `[?1049` sequence — asserted by the piped-output checks in
`test/cli.test.ts`. Whether the user's scrollback is genuinely intact after `q` is an **L4 human
gate**: it cannot be observed from inside the process.
