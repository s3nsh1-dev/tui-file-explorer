...
- `npm view <pkg> version` remains the way to *query* the registry — it needs no local install and
...
+ **Install-time code execution becomes an allowlist.** pnpm v10+ refuses to run dependency build
  ...
  control, not a promise. See the security posture in `v3_STAGE_3.md §4`.
...
− **Native modules need explicit approval.** `node-pty` (Stage 3, L3 evidence) ships `install`,
  ...
  *unbuilt* and fails at import time with an error that looks like a missing binding rather than a
...
− Peer dependencies must be declared explicitly. `ink@7` marks `react`, `@types/react`, and
  `react-devtools-core` as *optional* peers, so nothing installs them for us. `react` and
...
This ADR does not make the toolchain pnpm-only for *consumers*. The published tarball must install
