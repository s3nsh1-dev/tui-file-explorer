...
_Rejected:_ a `useEffect` that re-clamps the cursor after the list changes. It renders the invalid
state and then corrects it, which is a visible flash — and it splits one invariant across two places
that must agree.

_Rejected:_ keeping three `useState`s and being careful. "Being careful" is not a mechanism, and the
...
_Rejected:_ storing both an index and a name and keeping them in sync. Two sources of truth for one
...
on every _state change_ — and Ink re-renders on a frame timer, not only on input. `recompute()` is
documented as the sole writer.

_Rejected:_ `useMemo` in the component. Correct, conventional, and measurably worse at the scale this
...
The offset is _not_ in the reducer. It is `useState` in `App`, adjusted during render via the pure
